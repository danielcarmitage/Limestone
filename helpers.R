...
quandl_fx_download <- function(symbols, asset_name){
  quandl_fx_data <- Quandl(symbols, authcode = quandl_key)
  quandl_fx_data <- data.table(name = rep(asset_name, length(quandl_fx_data[,1])), date = quandl_fx_data[,1], rate = quandl_fx_data[,2], high = quandl_fx_data[,3], low = quandl_fx_data[,4])
...
quandl_raw_futures_list <- function(){
  cme_list_url <- "https://s3.amazonaws.com/quandl-static-content/Ticker+CSV%27s/Futures/CMEGroup.csv"
  ice_list_url <- "https://s3.amazonaws.com/quandl-static-content/Ticker+CSV%27s/Futures/ICE.csv"
  eurex_list_url <- "https://s3.amazonaws.com/quandl-static-content/Ticker+CSV%27s/Futures/EUREX.csv"
